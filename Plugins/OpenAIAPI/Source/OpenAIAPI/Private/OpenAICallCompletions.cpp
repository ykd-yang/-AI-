...
void UOpenAICallCompletions::Activate()
{
	...
	{
		UE_LOG(LogTemp, Warning, TEXT("[Completions] Failed to send HTTP request: URL=%s"), *HttpRequest->GetURL());
...
void UOpenAICallCompletions::OnResponse(FHttpRequestPtr Request, FHttpResponsePtr Response, bool WasSuccessful)
{
	...
	{
		UE_LOG(LogTemp, Warning, TEXT("[Completions] HTTP failure (WasSuccessful=%s, Status=%d, RequestURL=%s, ResponseURL=%s) Body=%s"),
	...
	UE_LOG(LogTemp, Log, TEXT("[Completions] HTTP %d OK (URL=%s)"), StatusCode, *ResponseUrl);
	...
	if (FJsonSerializer::Deserialize(reader, responseObject))
	{
		...
		if (err)
		{
			UE_LOG(LogTemp, Warning, TEXT("[Completions] API error JSON: %s"), *ResponseBody);
			Finished.Broadcast({}, TEXT("Api error: ") + ResponseBody, {}, false);
			return;
		}
	...
	{
		UE_LOG(LogTemp, Warning, TEXT("[Completions] JSON parse failed. Body=%s"), *ResponseBody);
		if (Finished.IsBound())
		{
